...
def Hospital_Forum(client, username, groupname):
	while True:
...
def handshake(client):
	...
	if groupname in groups:
		if username in groups[groupname].allMembers:
			...
			groups[groupname].sendMessage(username+" has requested to join the group.","Hospital_Forum")
			client.send(b"/wait")
			print("Join Request:",username,"| Group:",groupname)
		threading.Thread(target=Hospital_Forum, args=(client, username, groupname,)).start()
	else:
...
def main():
	if len(sys.argv) < 3:
		print("Add IP address and port number as arguements as well")
		
		return
	listenSocket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
	listenSocket.bind((sys.argv[1], int(sys.argv[2])))
	listenSocket.listen(10)
	print("Hospital_Forum Server running")
	while True:
